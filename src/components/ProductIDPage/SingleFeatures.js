...

            {
                data?.specification.map((Item, index) => (
                    <p key={index} className="grid grid-cols-3 px-3 py-2" style={{ borderBottom: '1px solid #ecedef' }}>
                        <span className="opacity-80 text-[12px] sm:text-[14px]">{Item.featureName}</span>
                        <span className="font-semibold col-span-2 md:col-span-1 text-[12px] sm:text-[14px]">{Item.feature}</span>
                    </p>
                ))
            }
